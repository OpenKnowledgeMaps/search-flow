...
function executeSearchRequest(
  ...
  $.ajax({
    ...
    .done(function (output) {
      if (output.status == "success") {
        return redirectToMap(vis_page, output.id, service, post_data);
      }
      errorOccurred();
      ...
      let current_error_texts =
        error_texts[search_flow_config.waiting_page_options.default_api_error];
...
function setErrorResolution(resolution, resolution_link, show_form = false) {
  if (show_form) {
